...
bool isMethodApplicable(float a[][n+1]){
    for(int i=0;i<n;i++){
       for(int j=0;j<n;j++){
        if(fabs(a[i][i])>findSum(i,a)) continue;
           else return false;   
        }
    }
    return true;
...
void print(int iteration,float values[n]){
    ...
    for(int i=0;i<n;i++) printf("value[%d]=%f ",i+1,values[i]);
    printf("\n");
}
void findValues(float a[][n+1],int maxIterations,float values_old[n]){
    ...
    float ratio,sum=0;
    float values_new[n];
    for(int i=0;i<n;i++) values_new[i]=0;
    for(iteration=1;iteration<=maxIterations;iteration++){
        ...
        for(k=0;k<n;k++){
            if(fabs(values_old[k]-values_new[k])<EPSILON) continue;          
            else{
               flag=1;
               break;
            }
        ...
        flag=0;       
        print(iteration,values_new);
        for(k=0;k<n;k++) values_old[k]=values_new[k];
    }
    print(iteration,values_new); 
}
int main(){
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n+1;j++) scanf("%f",&a[i][j]);
    }
